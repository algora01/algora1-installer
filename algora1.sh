...
zip_url_for_engine() {
  case "$1" in
    BEXP) echo "https://ce61ee09-0950-4d0d-b651-266705220b65.usrfiles.com/archives/ce61ee_238ba6fac47f44fbbb1e9495ba57ff00.zip" ;;
    PMNY) echo "https://ce61ee09-0950-4d0d-b651-266705220b65.usrfiles.com/archives/ce61ee_f1021e86eee840468af12cb3f83db49e.zip" ;;
    TSLA) echo "https://ce61ee09-0950-4d0d-b651-266705220b65.usrfiles.com/archives/ce61ee_d0a39aab412f4bc3badba3bea5003144.zip" ;;
    NVDA) echo "https://ce61ee09-0950-4d0d-b651-266705220b65.usrfiles.com/archives/ce61ee_24d2b535294f4479bf94a0c4bdf6eef7.zip" ;;
    *) echo "" ;;
  esac
}
...
install_local_cli() {
  ...
  # Prefer copying the running script file itself (preserves our latest version).
  # Falls back to GitHub download only when truly piped (no file on disk).
  local self=""
  if [ -f "${BASH_SOURCE[0]:-}" ] && [ "${BASH_SOURCE[0]:-}" != "$target" ]; then
    self="${BASH_SOURCE[0]}"
  elif [ -f "${0}" ] && [ "${0##*/}" != "bash" ] && [ "${0##*/}" != "sh" ]; then
    self="$(cd "$(dirname "$0")" && pwd)/$(basename "$0")"
  fi

  if [ -n "$self" ] && [ "$self" != "$target" ]; then
    cp -f "$self" "$target" 2>/dev/null || true
    chmod +x "$target"
  elif [ -z "$self" ] || [ "$self" = "$target" ]; then
    # Only download when there is no local file to copy from
    if [ "$self" != "$target" ]; then
      : "${ALGORA1_INSTALL_URL:=https://raw.githubusercontent.com/algora01/algora1-installer/main/algora1.sh}"
      need_cmd curl || true
      curl -fsSL "${ALGORA1_INSTALL_URL}" -o "${target}" 2>/dev/null || true
...
EOF
  ...
  # Also place a copy on the Desktop for easy access
  local desktop_app="${HOME}/Desktop/ALGORA1.app"
  rm -rf "$desktop_app" 2>/dev/null || true
  cp -R "$app_root" "$desktop_app" 2>/dev/null || true
...
write_exports_on_vm() {
  ...
  local _creds_script
  _creds_script="$(mktemp)"
  cat > "$_creds_script" <<'EOF'
set -euo pipefail
...
EOF
  ...
  ui_spin "Writing credentials to VM…" \
    ssh -i "${key_path}" -o StrictHostKeyChecking=accept-new \
    "${REMOTE_USER}@${ip}" bash -s -- \
    "${ALPACA_LIVE_API_KEY}" "${ALPACA_LIVE_SECRET_KEY}" \
    "${ALPACA_PAPER_API_KEY}" "${ALPACA_PAPER_SECRET_KEY}" < "$_creds_script"
  rm -f "$_creds_script" 2>/dev/null || true
...
EOF
  ...
  ui_spin "Customizing VM login banner…" \
    ssh -o LogLevel=ERROR -o StrictHostKeyChecking=accept-new -i "${key_path}" \
    "${REMOTE_USER}@${ip}" "bash -s" < "$_motd_script"
  rm -f "$_motd_script" 2>/dev/null || true
...
install_control_panel_on_vm() {
  ...
  # Write the remote install script to a temp file so it reaches SSH stdin
  # reliably even when gum spin intercepts the process's stdin.
  local _remote_script
  _remote_script="$(mktemp)"

  cat > "$_remote_script" <<'REMOTE_INSTALL_EOF'
set -euo pipefail
...
run_custom_engine_session() {
  ...
  local name
  echo "" >&2
  name="$(input "Session name (default: custom):")"
  name="${name:-custom}"
...
def parse_sma_lengths(rule):
    ...
    if len(nums) >= 3:
        return int(nums[1]), int(nums[2]), int(nums[0])
    return 10, 20, 50
...
st.set_page_config(page_title=f"ALGORA1 — {ENGINE_ID} Backtest", page_icon="📈",
...
def fetch_latest_price(ticker):
    try:
        t = yf.Ticker(ticker)
        return t.info.get("longName", ticker), float(t.history(period="1d")["Close"].iloc[-1])
    except Exception: return ticker, 0.0

@st.cache_data(ttl=300, show_spinner=False)
def fetch_spx(start, end):
    try:
        sp = yf.Ticker("^GSPC").history(start=pd.to_datetime(start),
            end=pd.to_datetime(end) + pd.Timedelta(days=2), interval="1d")
...
def run_strategy(ticker, sma_fast, sma_mid, sma_slow, stop_loss_pct):
    ...
    closed_trades = []; holding = False; buy_price = buy_date = stop_price = None
    for i in range(1, len(df)):
        curr_date = df.index[i]
        curr = bool(df["is_green"].iloc[i]); prev = bool(df["is_green"].iloc[i-1])
        close_p = float(df["close"].iloc[i]); low_p = float(df["low"].iloc[i])
        if not holding and curr and not prev:
            holding = True; buy_price = close_p; buy_date = curr_date
    ...
    if holding and buy_price is not None:
        open_trade = {"buy_date_str": buy_date.tz_localize(None).strftime("%Y-%m-%d"),
            "buy_price": buy_price, "latest_price": live_price,
            "unrealized_return": (live_price - buy_price) / buy_price * 100,
            "current_stop_price": stop_price}
    green = df[df["is_green"]]; out_of = df[~df["is_green"]]
...
def build_daily_nav(df, stop_loss_pct):
    equity = 10000.0; holding = False; shares = 0.0; buy_price = stop_price = None
...
def compute_metrics(nav):
    ...
    days = (nav.index[-1] - nav.index[0]).days; years = max(days / 365.25, 1e-9)
    cagr = (eq.iloc[-1] / eq.iloc[0]) ** (1 / years) - 1
    vol = r.std(ddof=0) * np.sqrt(252); mean = r.mean() * 252
    sharpe = mean / vol if vol > 0 else np.nan
    dn_vol = r[r < 0].std(ddof=0) * np.sqrt(252)
    sortino = mean / dn_vol if dn_vol > 0 else np.nan
    peak = eq.cummax(); max_dd = ((eq / peak) - 1.0).min()
    calmar = (cagr / abs(max_dd)) if max_dd < 0 else np.nan
    time_in = float(nav["in_market"].mean())
    rows = [("CAGR", f"{cagr*100:.2f}%"),
...
def annual_trade_exit(closed_trades):
    ...
    return df.groupby("Year").agg(
        **{"Compounded Return (%)": ("mult", lambda x: round((x.prod()-1)*100,2)),
           "Trades": ("mult","count")}).reset_index()

def spx_overlay(fig, start, end, exclude_year=2016):
    sp = fetch_spx(start, end)
    if sp is None or len(sp) < 2: return fig
    sp_ret = (sp["Close"] / sp["Close"].iloc[0] - 1.0) * 100.0
    sp_df = pd.DataFrame({"date": pd.to_datetime(sp_ret.index).tz_localize(None),
                           "ret": sp_ret.values}).sort_values("date")
    sp_df = sp_df[sp_df["date"].dt.year != exclude_year]
    if sp_df.empty: return fig
...
if not TICKERS:
    st.error("No tickers configured for this engine."); st.stop()
...
if len(TICKERS) > 1:
    ...
    if valid:
        frames = {t: v[0][["daily_ret"]].rename(columns={"daily_ret": f"r_{t}"}) for t, v in valid.items()}
        ...
        merged["in_mkt"] = sum((merged[f"r_{t}"] != 0).astype(float) * w for t, (_, w) in valid.items()) > 0
        port_nav = pd.DataFrame({"equity": merged["equity"], "daily_ret": merged["r_port"],
...
for ticker in TICKERS:
    if len(TICKERS) > 1: st.subheader(ticker)
    res = results[ticker]
    fig, closed_trades, open_trade, df_t = res["fig"], res["closed"], res["open"], res["df"]
    if fig is None or df_t is None:
        st.error(f"Could not load data for {ticker}."); continue
    st.plotly_chart(fig, use_container_width=True)
    if open_trade:
        st.markdown(
            f"<div style='background:#f0f0f0;border-radius:5px;padding:10px;color:#000;font-weight:bold;'>"
            ...
            f"</div><div style='margin-bottom:25px;'></div>", unsafe_allow_html=True)
    ...
    spx_ann = spx_annual(closed_f[0]["buy_date"].tz_localize(None),
                          closed_f[-1]["sell_date"].tz_localize(None) + pd.Timedelta(days=2), EXCLUDE_YEAR)
    ...
    nav_fig.add_trace(go.Scatter(x=nav_ex.index, y=nav_cum, mode="lines",
        name=f"{ticker} Strategy (Daily NAV / Mark-to-Market)", line=dict(color="#00ff00", width=3)))
...
REMOTE_INSTALL_EOF
  ...
  ui_spin "Installing control panel scripts on VM…" \
    ssh -o LogLevel=ERROR -o StrictHostKeyChecking=accept-new -i "${key_path}" \
    "${REMOTE_USER}@${ip}" "bash -s" < "$_remote_script"
